...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['customer-service-representative', 'owner', 'administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<DormitoryInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/dormitories` : null),
    () => getDormitories({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Dormitory',
      path: isTenantUser ? `/dormitories/view/${data?.[0]?.id}` : '/dormitories',
      entity: 'dormitory',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Booking', path: '/bookings', entity: 'booking', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
    { name: 'Offer', path: '/offers', entity: 'offer', service: AccessServiceEnum.PROJECT, icon: FiGift },
